...
def convert_pytorch_to_coreml(input_model_path, output_model_path):
    """
    ...
    Args:
        input_model_path: Path to the .pth file
        output_model_path: Path for the output .mlmodel file
    """
    ...
    mlmodel.license = "MIT"
    mlmodel.short_description = "MobileNetV2 model for eye disease detection (improved accuracy)"
    mlmodel.version = "2.0"
    ...
    print(f"Saving CoreML model to {output_model_path}...")
    mlmodel.save(output_model_path)
    
    print("Conversion completed successfully!")
    print(f"CoreML model saved to: {output_model_path}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 3:
        ...
        # Default paths for the new model
        input_path = "mobilenetv2(1).pth"
        output_path = "EyeDiseaseModelV2.mlmodel"
    ...
    try:
        ...
        if not os.path.exists(input_path):
            ...
            print("\nUsage:")
            print(f"  python {sys.argv[0]} <input.pth> <output.mlmodel>")
            print(f"\nOr place '{input_path}' in the current directory")
            sys.exit(1)
        
        convert_pytorch_to_coreml(input_path, output_path)
        print("=" * 60)
        ...
        print(f"1. Ensure '{input_path}' exists in the current directory")
